...
int main(int argc, char **argv) {
    ...
    while (running) {
        ...
        while (1) {
            ...
            render_sky(&sky_attrib, player, sky_buffer, g);
            glClear(GL_DEPTH_BUFFER_BIT);
            int face_count = render_chunks(&block_attrib, player, g);
            render_signs(&text_attrib, player, g);
            render_sign(&text_attrib, player, g);
            render_players(&block_attrib, player, g);
            if (SHOW_WIREFRAME) {
                render_wireframe(&line_attrib, player, g);
            }
            
            // RENDER HUD //
            glClear(GL_DEPTH_BUFFER_BIT);
            if (SHOW_CROSSHAIRS) {
                render_crosshairs(&line_attrib, g);
            }
            if (SHOW_ITEM) {
                render_item(&block_attrib, g);
            }
            ...
            if (SHOW_INFO_TEXT) {
                ...
                render_text(&text_attrib, ALIGN_LEFT, tx, ty, ts, text_buffer, g);
                ty -= ts * 2;
            }
            if (SHOW_CHAT_TEXT) {
            ...
            if (SHOW_PLAYER_NAMES) {
                if (player != me) {
                    render_text(&text_attrib, ALIGN_CENTER,
                                g->width / 2, ts, ts, player->name, g);
                }
                Player *other = player_crosshair(player, g);
                if (other) {
                    render_text(&text_attrib, ALIGN_CENTER,
            ...
            if (g->observe2) {
                ...
                render_sky(&sky_attrib, player, sky_buffer, g);
                glClear(GL_DEPTH_BUFFER_BIT);
                render_chunks(&block_attrib, player, g);
                render_signs(&text_attrib, player, g);
                render_players(&block_attrib, player, g);
                glClear(GL_DEPTH_BUFFER_BIT);
                if (SHOW_PLAYER_NAMES) {
                    render_text(&text_attrib, ALIGN_CENTER,
                                pw / 2, ts, ts, player->name, g);
                }
            }
